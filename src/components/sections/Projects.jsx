...
function RecommendationVisual({ hovered, reduced }) {
  const cards=[{t:"Action",x:"12%",y:"15%",r:-8},{t:"Sci-Fi",x:"58%",y:"10%",r:6},{t:"Drama",x:"5%",y:"55%",r:4},{t:"Comedy",x:"62%",y:"55%",r:-5}]
  return (
    <div style={{ width:"100%",height:"100%",position:"relative",background:"linear-gradient(135deg,rgba(167,139,250,0.06),rgba(124,58,237,0.08))",overflow:"hidden" }}>
      <div style={{ position:"absolute",width:110,height:110,borderRadius:"50%",background:"radial-gradient(circle,rgba(167,139,250,0.18),transparent 70%)",top:"50%",left:"50%",transform:"translate(-50%,-50%)",filter:"blur(16px)" }} aria-hidden="true"/>
      <div style={{ position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:40,height:40,borderRadius:"50%",zIndex:2,background:"linear-gradient(135deg,rgba(167,139,250,0.3),rgba(124,58,237,0.2))",border:"1.5px solid rgba(167,139,250,0.5)",boxShadow:hovered?"0 0 20px rgba(167,139,250,0.5)":"0 0 10px rgba(167,139,250,0.2)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,transition:"box-shadow 0.3s" }}>🎬</div>
      <svg style={{ position:"absolute",inset:0,width:"100%",height:"100%",zIndex:1 }} aria-hidden="true">
        {cards.map((c,i)=><line key={i} x1="50%" y1="50%" x2={c.x} y2={c.y} stroke={`rgba(167,139,250,${hovered?0.35:0.15})`} strokeWidth="1.5" strokeDasharray="4 3"/>)}
      </svg>
      {cards.map((c,i)=>(
        <motion.div key={c.t} style={{ position:"absolute",left:c.x,top:c.y,transform:`rotate(${c.r}deg)`,padding:"4px 10px",borderRadius:6,background:"rgba(167,139,250,0.12)",border:"1px solid rgba(167,139,250,0.3)",fontSize:11,fontWeight:600,color:"#A78BFA",zIndex:3 }}
          animate={!reduced?{y:[0,-5,0]}:{}} transition={{ duration:2.2+i*0.3,repeat:Infinity,ease:"easeInOut",delay:i*0.2 }}>{c.t}</motion.div>
      ))}
      <span style={{ position:"absolute",bottom:10,left:12,fontSize:10,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",color:"rgba(167,139,250,0.6)" }}>CF + CBF · Hybrid</span>
    </div>
  )
}
function ChatVisual({ hovered, reduced }) {
  const msgs=[{text:"Hello! How can I help?",bot:true},{text:"Tell me about ML.",bot:false},{text:"ML learns from data...",bot:true},{text:"That's amazing! 🚀",bot:false}]
  return (
    <div style={{ width:"100%",height:"100%",position:"relative",background:"linear-gradient(135deg,rgba(56,189,248,0.06),rgba(59,130,246,0.08))",overflow:"hidden",padding:"14px 16px",display:"flex",flexDirection:"column",gap:7,justifyContent:"center" }}>
      <div style={{ position:"absolute",width:100,height:100,borderRadius:"50%",background:"radial-gradient(circle,rgba(56,189,248,0.15),transparent 70%)",bottom:"-10%",right:"-5%",filter:"blur(14px)" }} aria-hidden="true"/>
      <div style={{ position:"absolute",top:10,right:14,width:28,height:28,borderRadius:"50%",background:"linear-gradient(135deg,#38BDF8,#3B82F6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,boxShadow:hovered?"0 0 12px rgba(56,189,248,0.5)":"none",transition:"box-shadow 0.3s" }}>🤖</div>
      {msgs.map((m,i)=>(
        <motion.div key={i} initial={reduced?{}:{opacity:0,x:m.bot?-12:12}} whileInView={{opacity:1,x:0}} viewport={{once:true}} transition={{ delay:0.1+i*0.12,duration:0.4 }}
          style={{ alignSelf:m.bot?"flex-start":"flex-end",maxWidth:"78%",padding:"6px 11px",borderRadius:m.bot?"4px 12px 12px 12px":"12px 4px 12px 12px",background:m.bot?"rgba(56,189,248,0.12)":"rgba(255,255,255,0.07)",border:`1px solid ${m.bot?"rgba(56,189,248,0.28)":"rgba(255,255,255,0.1)"}`,fontSize:11,color:m.bot?"#38BDF8":"#CBD5E0",fontWeight:500,lineHeight:1.4 }}>
          {m.text}
        </motion.div>
      ))}
      <span style={{ position:"absolute",bottom:10,left:12,fontSize:10,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",color:"rgba(56,189,248,0.6)" }}>Rule-Based · Context-Aware</span>
    </div>
  )
}
function TechGraphVisual({ hovered, reduced }) {
  const nodes=[{label:"React",x:"15%",y:"20%",c:"#61DAFB"},{label:"Python",x:"65%",y:"15%",c:"#FFD43B"},{label:"FastAPI",x:"75%",y:"60%",c:"#34D399"},{label:"TF",x:"10%",y:"65%",c:"#FF6F00"},{label:"SQL",x:"45%",y:"72%",c:"#38BDF8"}]
  return (
    <div style={{ width:"100%",height:"100%",position:"relative",background:"linear-gradient(135deg,rgba(52,211,153,0.06),rgba(5,150,105,0.08))",overflow:"hidden" }}>
      <div style={{ position:"absolute",width:90,height:90,borderRadius:"50%",background:"radial-gradient(circle,rgba(52,211,153,0.18),transparent 70%)",top:"30%",left:"35%",filter:"blur(14px)" }} aria-hidden="true"/>
      <div style={{ position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:44,height:44,borderRadius:"50%",zIndex:2,background:"linear-gradient(135deg,rgba(52,211,153,0.25),rgba(5,150,105,0.15))",border:"1.5px solid rgba(52,211,153,0.5)",boxShadow:hovered?"0 0 20px rgba(52,211,153,0.45)":"0 0 8px rgba(52,211,153,0.2)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,transition:"box-shadow 0.3s" }}>⚙️</div>
      <svg style={{ position:"absolute",inset:0,width:"100%",height:"100%",zIndex:1 }} aria-hidden="true">
        {nodes.map((n,i)=><line key={i} x1="50%" y1="50%" x2={n.x} y2={n.y} stroke={`rgba(52,211,153,${hovered?0.35:0.15})`} strokeWidth="1.2" strokeDasharray="3 3"/>)}
      </svg>
      {nodes.map((n,i)=>(
        <motion.div key={n.label} style={{ position:"absolute",left:n.x,top:n.y,transform:"translate(-50%,-50%)",padding:"3px 9px",borderRadius:6,zIndex:3,background:`${n.c}14`,border:`1px solid ${n.c}35`,fontSize:10,fontWeight:700,color:n.c }}
          animate={!reduced?{scale:[1,1.08,1]}:{}} transition={{ duration:2.5+i*0.3,repeat:Infinity,ease:"easeInOut",delay:i*0.25 }}>{n.label}</motion.div>
      ))}
      <span style={{ position:"absolute",bottom:10,left:12,fontSize:10,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",color:"rgba(52,211,153,0.6)" }}>TF-IDF · Cosine Similarity</span>
    </div>
  )
}
const VISUALS = { 1:WaveformVisual, 2:RecommendationVisual, 3:ChatVisual, 4:TechGraphVisual }
